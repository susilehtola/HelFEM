...
namespace helfem {
  namespace polynomial_basis {
    ...
    void drop_last(arma::uvec &idx, int noverlap) {
      // This is simple too - drop the last noverlap functions to force function
      // and its derivatives to zero
      idx = idx.subvec(0, idx.n_elem - noverlap - 1);
    }

    arma::uvec primitive_indices(int nprim, int noverlap, bool first,
                                 bool last) {
    ...
    PolynomialBasis *get_basis(int primbas, int Nnodes) {
      if (Nnodes < 2)
        throw std::logic_error("Can't have finite element basis with less than "
      ...
      case (2):
        ...
        printf("Basis set composed of %i nodes with %i:th derivative "
               "continuity.\n",
               Nnodes, primbas);
    ...
    int PolynomialBasis::get_nbf() const { return nbf; }

    int PolynomialBasis::get_noverlap() const { return noverlap; }

    int PolynomialBasis::get_id() const { return id; }

    int PolynomialBasis::get_order() const { return order; }
    ...
    void PolynomialBasis::eval_lapl(const arma::vec &x, arma::mat &lf) const {
      throw std::logic_error("Laplacians haven't been implemented for the used "
                             "family of basis polynomials.\n");
    }
    ...
    HermiteBasis *HermiteBasis::copy() const { return new HermiteBasis(*this); }

    arma::mat HermiteBasis::eval(const arma::vec &x) const {
      return polynomial::polyval(bf_C, x);
    }

    void HermiteBasis::eval(const arma::vec &x, arma::mat &f,
                            arma::mat &df) const {
    ...
    void HermiteBasis::drop_first() {
      ...
      arma::uvec idx(
          arma::linspace<arma::uvec>(0, bf_C.n_cols - 1, bf_C.n_cols));
    ...
    LegendreBasis::LegendreBasis(int n_nodes, int id_) {
      ...
      // Shape functions [Flores, Clementi, Sonnad, Chem. Phys. Lett. 163, 198
      // (1989)]
    ...
    void LegendreBasis::eval(const arma::vec &x, arma::mat &f,
                             arma::mat &df) const {
      f = f_eval(x) * T;
      df = df_eval(x) * T;
    }
    ...
    LIPBasis *LIPBasis::copy() const { return new LIPBasis(*this); }
