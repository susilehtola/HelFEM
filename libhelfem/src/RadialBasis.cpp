...
namespace helfem {
  namespace atomic {
    namespace basis {
      ...
      RadialBasis::RadialBasis(const polynomial_basis::PolynomialBasis *poly_,
                               int n_quad, const arma::vec &bval_) {
        // Polynomial basis
        poly = poly_->copy();
      ...
      RadialBasis::get_basis(const polynomial_basis::PolynomialBasis *polynom,
                             size_t iel) const {
      ...
      size_t RadialBasis::Nbf() const {
        // Number of basis functions is Nprim*Nel - (Nel-1)*noverlap - 1 -
        // noverlap
      ...
      void RadialBasis::get_idx(size_t iel, size_t &ifirst,
                                size_t &ilast) const {
      ...
      arma::mat RadialBasis::radial_integral(const arma::mat &funcs, int Rexp,
                                             size_t iel) const {
        double Rmin(bval(iel));
        double Rmax(bval(iel + 1));

        // Integral by quadrature
        return quadrature::radial_integral(Rmin, Rmax, Rexp, xq, wq,
                                           get_basis(funcs, iel));
      }

      arma::mat RadialBasis::bessel_il_integral(int L, double lambda,
                                                size_t iel) const {
        double Rmin(bval(iel));
        double Rmax(bval(iel + 1));

        // Integral by quadrature
        return quadrature::bessel_il_integral(Rmin, Rmax, L, lambda, xq, wq,
                                              get_basis(bf, iel));
      }

      arma::mat RadialBasis::bessel_kl_integral(int L, double lambda,
                                                size_t iel) const {
        double Rmin(bval(iel));
        double Rmax(bval(iel + 1));

        // Integral by quadrature
        return quadrature::bessel_kl_integral(Rmin, Rmax, L, lambda, xq, wq,
                                              get_basis(bf, iel));
      }

      arma::mat RadialBasis::radial_integral(const RadialBasis &rh, int n,
      ...
      arma::mat
      RadialBasis::model_potential(const RadialBasis &rh,
        ...
        for (size_t iel = 0; iel < bval.n_elem - 1; iel++) {
          ...
          for (size_t jel = 0; jel < rh.bval.n_elem - 1; jel++) {
            ...
            if ((jstart >= istart && jstart < iend) ||
              ...
              // printf("New element %i overlaps with old element
              // %i\n",iel,jel);
            }
          }
        }
        ...
        for (size_t iel = 0; iel < bval.n_elem - 1; iel++) {
          ...
          for (size_t jj = 0; jj < overlap[iel].size(); jj++) {
            ...
            arma::vec r(intmid * arma::ones<arma::vec>(xproj.n_elem) +
                        intlen * xproj);
      ...
      arma::mat
      RadialBasis::model_potential(const modelpotential::ModelPotential *model,
                                   size_t iel) const {
        double Rmin(bval(iel));
        double Rmax(bval(iel + 1));
      ...
      arma::mat RadialBasis::nuclear_offcenter(size_t iel, double Rhalf,
                                               int L) const {
        if (bval(iel) >= Rhalf)
          return -sqrt(4.0 * M_PI / (2 * L + 1)) *
                 radial_integral(-L - 1, iel) * std::pow(Rhalf, L);
      ...
      arma::mat RadialBasis::yukawa_integral(int L, double lambda,
                                             size_t iel) const {
        double Rmin(bval(iel));
        double Rmax(bval(iel + 1));
        ...
        polynomial_basis::PolynomialBasis *p(get_basis(poly, iel));
        arma::mat tei(
            quadrature::yukawa_integral(Rmin, Rmax, xq, wq, p, L, lambda));
        delete p;

        return tei;
      }

      arma::mat RadialBasis::erfc_integral(int L, double mu, size_t iel,
                                           size_t kel) const {
        ...
        for (size_t ii = 0; ii < Nint; ii++) {
          ...
          xk.subvec(ii * Nq, (ii + 1) * Nq - 1) =
              arma::ones<arma::vec>(Nq) * imid + xi * ilen;
        ...
        // Evaluate integral
        arma::mat tei(quadrature::erfc_integral(
            Rmini, Rmaxi, get_basis(ibf, iel), xi, wi, Rmink, Rmaxk,
            get_basis(kbf, kel), xk, wk, L, mu));
      ...
      double RadialBasis::nuclear_density(const arma::mat &Prad) const {
        if (Prad.n_rows != Nbf() || Prad.n_cols != Nbf())
          throw std::logic_error(
              "nuclear_density expects a radial density matrix\n");
      ...
      double
      RadialBasis::nuclear_density_gradient(const arma::mat &Prad) const {
        if (Prad.n_rows != Nbf() || Prad.n_cols != Nbf())
          throw std::logic_error(
              "nuclear_density_gradient expects a radial density matrix\n");
