...
namespace helfem {
  namespace quadrature {
    arma::mat radial_integral(double rmin, double rmax, int n,
                              const arma::vec &x, const arma::vec &wx,
                              const arma::mat &bf) {
    ...
    arma::mat derivative_integral(double rmin, double rmax, const arma::vec &x,
                                  const arma::vec &wx, const arma::mat &dbf) {
#ifndef ARMA_NO_DEBUG
      if (x.n_elem != wx.n_elem) {
        std::ostringstream oss;
        oss << "x and wx not compatible: " << x.n_elem << " vs " << wx.n_elem
    ...
    arma::mat model_potential_integral(
        double rmin, double rmax, const modelpotential::ModelPotential *nuc,
        const arma::vec &x, const arma::vec &wx, const arma::mat &bf) {
#ifndef ARMA_NO_DEBUG
      if (x.n_elem != wx.n_elem) {
        std::ostringstream oss;
        oss << "x and wx not compatible: " << x.n_elem << " vs " << wx.n_elem
    ...
    arma::mat bessel_il_integral(double rmin, double rmax, int L, double lambda,
                                 const arma::vec &x, const arma::vec &wx,
                                 const arma::mat &bf) {
#ifndef ARMA_NO_DEBUG
      if (x.n_elem != wx.n_elem) {
    ...
    arma::mat bessel_kl_integral(double rmin, double rmax, int L, double lambda,
                                 const arma::vec &x, const arma::vec &wx,
                                 const arma::mat &bf) {
#ifndef ARMA_NO_DEBUG
      if (x.n_elem != wx.n_elem) {
    ...
    static arma::vec twoe_inner_integral_wrk(
        double rmin, double rmax, double rmin0, double rmax0,
        const arma::vec &x, const arma::vec &wx,
        const polynomial_basis::PolynomialBasis *poly, int L) {
      // Midpoint is at
      double rmid(0.5 * (rmax + rmin));
      // and half-length of interval is
      double rlen(0.5 * (rmax - rmin));
    ...
    arma::mat twoe_inner_integral(double rmin, double rmax, const arma::vec &x,
                                  const arma::vec &wx,
      ...
      inner.row(0) = arma::trans(
          twoe_inner_integral_wrk(rmin, r(0), rmin, rmax, x, wx, poly, L));
      // Every subinterval uses a fresh nquad points!
      for (size_t ip = 1; ip < x.n_elem; ip++)
        inner.row(ip) = inner.row(ip - 1) +
                        arma::trans(twoe_inner_integral_wrk(
                            r(ip - 1), r(ip), rmin, rmax, x, wx, poly, L));
    ...
    arma::mat twoe_integral(double rmin, double rmax, const arma::vec &x,
                            const arma::vec &wx,
                            const polynomial_basis::PolynomialBasis *poly,
                            int L) {
#ifndef ARMA_NO_DEBUG
    ...
    static arma::vec yukawa_inner_integral_wrk(
        double rmin, double rmax, double rmin0, double rmax0,
        const arma::vec &x, const arma::vec &wx,
        const polynomial_basis::PolynomialBasis *poly, int L, double lambda) {
      // Midpoint is at
      double rmid(0.5 * (rmax + rmin));
      // and half-length of interval is
      double rlen(0.5 * (rmax - rmin));
    ...
    arma::mat yukawa_inner_integral(
        double rmin, double rmax, const arma::vec &x, const arma::vec &wx,
        const polynomial_basis::PolynomialBasis *poly, int L, double lambda) {
      // Midpoint is at
      double rmid(0.5 * (rmax + rmin));
      // and half-length of interval is
      ...
      // Compute the "inner" integrals as function of r.
      arma::mat inner(x.n_elem, std::pow(poly->get_nbf(), 2));
      inner.row(0) = arma::trans(yukawa_inner_integral_wrk(
          rmin, r(0), rmin, rmax, x, wx, poly, L, lambda));
      // Every subinterval uses a fresh nquad points!
      for (size_t ip = 1; ip < x.n_elem; ip++)
        inner.row(ip) =
            inner.row(ip - 1) +
            arma::trans(yukawa_inner_integral_wrk(r(ip - 1), r(ip), rmin, rmax,
                                                  x, wx, poly, L, lambda));
    ...
    arma::mat yukawa_integral(double rmin, double rmax, const arma::vec &x,
                              const arma::vec &wx,
                              const polynomial_basis::PolynomialBasis *poly,
                              int L, double lambda) {
#ifndef ARMA_NO_DEBUG
      ...
      // Compute the inner integrals
      arma::mat inner(
          yukawa_inner_integral(rmin, rmax, x, wx, poly, L, lambda));
    ...
    arma::mat erfc_integral(double rmini, double rmaxi, const arma::mat &bfi,
                            const arma::vec &xi, const arma::vec &wi,
                            double rmink, double rmaxk, const arma::mat &bfk,
                            const arma::vec &xk, const arma::vec &wk, int L,
                            double mu) {
#ifndef ARMA_NO_DEBUG
    ...
    arma::mat
    spherical_potential(double rmin, double rmax, const arma::vec &x,
                        const arma::vec &wx,
                        const polynomial_basis::PolynomialBasis *poly) {
      // Midpoint is at
      double rmid(0.5 * (rmax + rmin));
      ...
      for (size_t ip = 0; ip < x.n_elem; ip++) {
        ...
        double high = r(ip);
        zero.col(ip) =
            twoe_inner_integral_wrk(low, high, rmin, rmax, x, wx, poly, 0);
      }
      for (size_t ip = 0; ip < x.n_elem; ip++) {
        double low = r(ip);
        double high = (ip == x.n_elem - 1) ? rmax : r(ip + 1);
        minusone.col(ip) =
            twoe_inner_integral_wrk(low, high, rmin, rmax, x, wx, poly, -1);
      }
