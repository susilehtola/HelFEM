    ...
    namespace erfc_expn {
      ...
      inline static double Fn(unsigned int n, double Xi, double xi) {
        ...
        for (unsigned int p = 0; p <= n; p++) {
          F += std::pow(prefac, p + 1) *
               (factorial(n + p) / (factorial(p) * factorial(n - p))) *
               (std::pow(-1, n - p) * explus - exminus);
        }
        // Apply prefactor
      ...
      inline static double Hn(unsigned int n, double Xi, double xi) {
        ...
        double Hn = (Xi2np1 + xi2np1) * erfc(Xi + xi) -
                    (Xi2np1 - xi2np1) * erfc(Xi - xi);
      ...
      double Dnk(int n, int k, double Xi) {
        // Prefactor
        double prefac = std::exp(-std::pow(Xi, 2)) / sqrt(M_PI) *
                        std::pow(2, n + 1) * std::pow(Xi, 2 * n + 1);
        ...
        if (k == 0) {
          ...
          double sum = 0.0;
          for (int m = 1; m <= n; m++)
            sum += 1.0 / (double_factorial(2 * (n - m) + 1) *
                          std::pow(2 * Xi * Xi, m));
          ...
          double sum = 0.0;
          for (int m = 1; m <= k; m++)
            sum += choose(m - k - 1, m - 1) * std::pow(2 * Xi * Xi, k - m) /
                   double_factorial(2 * (n + k - m) + 1);

          D = prefac * (2.0 * n + 1.0) /
              (factorial(k) * (2.0 * (n + k) + 1.0)) * sum;
        }
      ...
      double Phi_short(unsigned int n, double Xi, double xi) {
        ...
        if (std::abs(dPhi) >= tol * std::abs(Phi))
          fprintf(stderr, "Warning - short-range Phi not converged, ratio %e\n",
                  dPhi / Phi);
        // fprintf(stderr,"Phi%i(%e,%e) converged in %u
        // iterations\n",n,Xi,xi,k);
