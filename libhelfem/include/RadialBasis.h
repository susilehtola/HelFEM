  ...
  namespace atomic {
    namespace basis {
      ...
      public:
        ...
        /// Radial multipole potential generated by a density on this basis:
        ///
        ///     V^k(r) = integral  (r_<^k / r_>^(k+1))  rho(r') dr'
        ///
        /// with rho given by the density matrix D in this basis, i.e.
        /// rho(r') dr' = D_uv B_u(r') B_v(r') dr' in the same convention
        /// radial_integral uses (so k = 0 reproduces the monopole potential
        /// behind coulomb_screening).
        ///
        /// Returned as a CALLABLE rather than as values on this basis's
        /// quadrature nodes, so that a caller can integrate it against
        /// functions living on a DIFFERENT grid:
        ///
        ///     lh.matrix_element(rh, B0, B0, lh.multipole_potential(D, k))
        ///
        /// is the Coulomb matrix of a frozen density D in an arbitrary other
        /// basis, with no projection of one basis onto the other. That
        /// matters for functions the reference grid cannot represent: a
        /// Gaussian with alpha ~ 1e6 has support ~1e-3 a0 and projecting it
        /// destroys its norm, while the integral against the potential is
        /// perfectly well defined -- in the alpha -> infinity limit it simply
        /// samples V^k at the nucleus.
        ///
        /// The returned closure holds a pointer to this basis and must not
        /// outlive it. Each evaluation costs one sub-element quadrature, the
