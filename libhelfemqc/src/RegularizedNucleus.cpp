/*
 *                This source code is part of
 *
 *                          HelFEM
 *                             -
 * Finite element methods for electronic structure calculations on small systems
 *
 * Written by Susi Lehtola, 2018-
 * Copyright (c) 2018- Susi Lehtola
 *
 * SPDX-License-Identifier: BSD-3-Clause
 * See the LICENSE file at the root of this source distribution
 * for the full license text.
 */
#include "RegularizedNucleus.h"
#include "Matrix.h"
#include "utils.h"
// Scalar formatter that prints a_ T value at its own precision (no truncation
// to double). Header-only, needs only Matrix.h + std; see src/general/eigen_io.h.
#include "../../src/general/eigen_io.h"
#include <chebyshev.h>
#include <cmath>
#include <functional>
#include <limits>
#include <stdexcept>

namespace helfem {
  namespace modelpotential {
    template <typename T>
    static T phi(T a_, T b_, T r) {
      T h=-r*std::erf(a_*r)-b_*std::exp(-std::pow(a_*r,2));
      return std::exp(h)/std::sqrt(utils::pi<T>());
    }

    template <typename T>
    static T normalization_slice(int N, std::function<T(T)> & phif) {
      // Get quadrature rule
      helfem::Vec<T> x, w;
      helfem::chebyshev::chebyshev<T>(N, x, w);

      // Evaluate normalization by adaptive quadrature
      T h = T(1e-1); // Slice size
      // Integrand
      helfem::Vec<T> f(N);

      T norm=T(0);
      size_t islice=0;
      while(true) {
        // Integration limits
        T r_middle = (islice+T(0.5))*h;
        // Compute function
        for(int i=0;i<N;i++) {
          T r = r_middle + T(0.5)*x(i)*h;
          f(i) = r*phif(r);
        }
        // Do quadrature
        T slice = T(2)*h*utils::pi<T>()*f.cwiseProduct(f).dot(w);
        norm += slice;
        if(slice == T(0)) {
          break;
        }
        islice++;
      }

      return norm;
    }

    template <typename T>
    static T normalization_chebyshev(int N, std::function<T(T)> & phif) {
      // Get quadrature rule
      helfem::Vec<T> r, wr;
      helfem::chebyshev::radial_chebyshev<T>(N, r, wr);

      // Integrand
      helfem::Vec<T> f(N);
      for(int i=0;i<N;i++) {
        f(i) = r(i)*phif(r(i));
      }

      return T(4)*utils::pi<T>()*f.cwiseProduct(f).dot(wr);
    }

    template <typename T>
    static T normalization(int N, std::function<T(T)> & phif) {
      return normalization_chebyshev<T>(N, phif);
      //return normalization_slice<T>(N, phif);
    }

    template <typename T>
    static T phi_normalization(T a_, T b_, int N) {
      std::function<T(T)> phif = [a_,b_](T r) {return phi<T>(a_,b_,r);};
      return normalization<T>(N,phif);
    }

    template <typename T>
    static T find_b(T a_, int N) {
      // Find b_ by bracketing; we want the normalization to be
      const T target=T(1);

      // The normalization behaves non-linearly in b_, so we need to
      // first bracket the location of the targeted value with a_ brute
      // force search
      size_t Npts=1000;
      helfem::Mat<T> norm(Npts,2);
      // logspace(-3, 1, Npts): Npts points logarithmically spaced between
      // 10^-3 and 10^1. Eigen has no logspace, so build via linspace * ln(10).
      norm.col(0) = (helfem::Vec<T>::LinSpaced(Npts, T(-3), T(1)).array()
                        * std::log(T(10))).exp().matrix();
      for(size_t ip=0; ip<Npts; ip++)
        norm(ip,1) = phi_normalization<T>(a_,norm(ip,0),N);

      // Find the value that is closest to the target
      const helfem::Vec<T> dnorm = (norm.col(1).array() - target).abs().matrix();
      Eigen::Index idx;
      dnorm.minCoeff(&idx);

      T b_right=norm(idx+1,0);
      if(phi_normalization<T>(a_,b_right,N) > target)
        throw std::logic_error("Could not right bracket normalized wf!\n");
      T b_left=norm(idx-1,0);
      if(phi_normalization<T>(a_,b_left,N) < target)
        throw std::logic_error("Could not left bracket normalized wf!\n");

      T b_middle;
      while(true) {
        b_middle = (b_left+b_right)/T(2);
        // Shadows the bracketing table above in the original; renamed.
        T bnorm = phi_normalization<T>(a_,b_middle, N);
        if(bnorm == target || std::abs(b_right-b_left) <= T(10)*std::numeric_limits<T>::epsilon()*b_middle) {
          break;
        } else if(bnorm > target) {
          b_left = b_middle;
        } else {
          b_right = b_middle;
        }
      }

      return b_middle;
    }

    template <typename T>
    RegularizedNucleusT<T>::RegularizedNucleusT(int Z_, T a) : Z(Z_) {
      // a_ and b_ are initialized by set_a
      set_a(a);
    }

    template <typename T>
    RegularizedNucleusT<T>::~RegularizedNucleusT() {
    }

    /* Autogenerated C code from Maple with the input

       with(CodeGeneration);
       h:=r->-r*erf(a_*r)-b_*exp(-a_^2*r^2);
       phi:=r->1/sqrt(Pi)*exp(h(r));
       V:=r->expand(simplify(-1/2 + 1/(2*phi(r)*r)*diff(r*phi(r),r,r)));
       (* Analytic expression *)
       C(V(r), declare = [r::numeric], optimize);

       (* Taylor series at r=0 *)
       p:=r->simplify(convert(series(V(r),r=0,8),polynom));
       C(p(r), declare = [r::numeric], optimize);

       The rational constants Maple emitted as double literals (0.44e2 /
       0.15e2 and friends) are spelled T(44)/T(15) here so that they are
       formed at the working precision rather than computed in double and
       promoted. Each one is an exact ratio of small integers, so the
       double instantiation is bit-identical to the pre-template code.
    */

    template <typename T>
    static T V_analytic(T a_, T b_, T r) {
      T t1 = a_ * a_;
      T t3 = r * r;
      T t5 = std::exp(t3 * t1);
      T t6 = T(1) / t5;
      T t8 = std::erf(a_ * r);
      T t13 = std::sqrt(utils::pi<T>());
      T t14 = T(1) / t13;
      T t20 = t14 * t3;
      T t21 = t1 * a_;
      T t28 = t1 * t1;
      T t29 = t28 * t3;
      T t36 = t5 * t5;
      T t37 = T(1) / t36;
      T t38 = b_ * b_;
      T t51 = t8 * t8;
      return -T(2) * b_ * t8 * t6 * t1 * r + T(2) * t8 * t6 * a_ * t14 * r + T(2) * t6 * t21 * t20 - T(4) * t6 * a_ * t14 - T(2) * b_ * t6 * t29 + T(3) * b_ * t6 * t1 + T(2) * t38 * t37 * t29 - T(4) * b_ * t37 * t21 * t20 + T(2) * t37 * t1 / utils::pi<T>() * t3 + t51 / T(2) - T(1) / T(2) - t8 / r;
    }

    template <typename T>
    static T V_taylor(T a_, T b_, T r) {
      T t1 = r * r;
      T t2 = t1 * t1;
      T t3 = t2 * t1;
      T t6 = a_ * a_;
      T t7 = t6 * t6;
      T t8 = t7 * t7;
      T t13 = t7 * t6;
      T t24 = std::sqrt(utils::pi<T>());
      T t25 = t24 * utils::pi<T>();
      return T(4) / t25 * (t25 * (-T(1) / T(8) + t8 * (b_ - T(3) / T(8)) * t3 * b_ - t13 * b_ * (b_ - T(7) / T(8)) * t2 + t7 * (b_ - T(5) / T(2)) * b_ * t1 / T(2) + T(3) / T(4) * b_ * t6) - T(44) / T(15) * a_ * (t24 * (-T(47) / T(66) * t3 * t7 * a_ + T(10) / T(11) * t2 * t6 * a_ - T(15) / T(22) * t1 * a_) + (T(45) / T(88) + t13 * (b_ - T(45) / T(308)) * t3 - T(25) / T(22) * t7 * t2 * (b_ - T(63) / T(200)) + T(15) / T(22) * t6 * (b_ - T(5) / T(6)) * t1) * utils::pi<T>()));
    }

    template <typename T>
    T RegularizedNucleusT<T>::V(T r) const {
      /* Gygi eq. 18: V(Z,r) = Z^2 V(1, Zr) */
      T Zr=Z*r;
      T val;
      if(Zr <= std::cbrt(std::numeric_limits<T>::epsilon())) {
        val=modelpotential::V_taylor<T>(a_,b_,Zr);
      } else {
        val=modelpotential::V_analytic<T>(a_,b_,Zr);
      }
      return std::pow(T(Z),2)*val;
    }

    template <typename T>
    T RegularizedNucleusT<T>::a() const {
      return a_;
    }

    template <typename T>
    T RegularizedNucleusT<T>::b() const {
      return b_;
    }

    template <typename T>
    void RegularizedNucleusT<T>::set_a(T a) {
      a_ = a;

      // Number of quadrature points
      int N = 100;
      b_ = find_b<T>(a_,N);
      // Format the T values at their own precision (no truncation to double).
      printf("a_ = %s yields b_ = %s\n",helfem::io::fmt_sci(a_).c_str(),helfem::io::fmt_sci(b_).c_str());
    }

    template class RegularizedNucleusT<double>;
    template class RegularizedNucleusT<long double>;
#ifdef HELFEM_HAVE_FLOAT128
    template class RegularizedNucleusT<_Float128>;
#endif
  }
}
